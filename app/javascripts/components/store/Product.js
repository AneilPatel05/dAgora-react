...
class Product extends Component {
  constructor(props) {
    super(props);
    this.state = {dAgora: props.dAgora, product: props.product};
  }
    ...
    <div className="col-md-4 text-center">
      <div className="well product product-home">
        <div className="ribbon">
          <span>&Xi; {web3.fromWei(parseFloat(this.state.product[4]), "ether")}</span>
        </div>
        <img src="https://placeimg.com/150/150/any" className="img-rounded hvr-grow" />
        <h3>{this.state.product[1]}</h3>
        <h4>{this.state.product[2]}</h4>
        <h6><b>Stock</b>: {parseFloat(this.state.product[5])}</h6>
  ...
  handlePurchase = (event) => {
    ...
    da.productList.call(_product[0]).then(function(product) {
      console.log(product);
      return da.buy( _product[0], {from: web3.eth.defaultAccount, gas: 3000000, value: parseFloat(product[4])} );
    }).then(function(tx_id) {
      console.log(tx_id);
